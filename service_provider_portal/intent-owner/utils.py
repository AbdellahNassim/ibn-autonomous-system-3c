...
def setup_logger():
    ...
    # set logger name
    log = logging.getLogger("Intent Owner")
...
def save_intent(session, logger, user_intent):
    ...
    service_type = session.query(ServiceType).filter(
        ServiceType.name == user_intent['service_type']).first()
    ...
    user = session.query(User).filter(
        User.id == user_intent['user_id']).first()
    ...
    if service_type.name == 'video':
        ...
        video_service_params = VideoServiceParams(service=service,
                                                  ...
                                                  throughput=user_intent['throughput']
                                                  )
        ...
        intent = Intent(id=intent_id,
                        ...
                        standard_format=standard_intent
                        )
        # persisting
        session.add(service)
...
def send_intent_backend(logger, user_intent):
    ...
    if response.status_code != 200:
        raise Exception(
            f"An error occurred while sending the intent {response.status_code} was received")
