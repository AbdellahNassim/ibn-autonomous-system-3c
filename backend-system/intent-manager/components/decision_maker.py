    ...
    while True: 
        try:
            if not queue.empty():
                ...
                predictions = execute_model_intent_params(logger, ml_model, intent)
                # generating decisions now 
                generate_deploy_decision(logger, service, predictions)
...
def fetch_model_marketplace(logger, intent):
    ...
    response = requests.get(os.environ['ML_MARKET_PLACE_HOST']+'/models/search', json=data)
    if response.status_code != 200:
        ...
        logger.info('Search resulted successfully, model with id {} will be fetched'.format(model_id))
        # now get the model 
        response = requests.get(os.environ['ML_MARKET_PLACE_HOST']+'/models/'+model_id)
        if response.status_code != 200:
...
def generate_deploy_decision(logger, service, resources):
    """
        Generate Deployment decision and send to the policy generator
    """
    # generate decision 
    logger.info('Generating decision to deploy {}'.format(service))
    logger.info('Generating decision to allocate {}'.format(resources))